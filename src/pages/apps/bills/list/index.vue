...
// const response = await $api(`/get-all-clientes`, { method: "GET" });
// const clientes = computed(() => response);
const filtros = ref({
    search: "",
    cliente: { value: null },
    fecha: { start: "", end: "" }
...
const importe = ref(0); // suma de los toales de las facturas
    ...
    try {
        const response = await $api("/get-facturas", {
            ...
            params: {
                ...
                // id_cliente: filtros.value.cliente.value,
        ...
        totalFacturas.value = response.total;
...
const deleteFactura = async () => {
    ...
    await $api(`/delete-factura`, {
        method: "POST",
        body: selectedFactura.value
    });
    loadings.value[0] = false;
    isDeleteFacturaVisible.value = false;
...
const exportar = async () => {
    try {
        ...
        const response = await $api(`/exportar-facturas`, {
            method: "GET",
            params: {
                search: filtros.value.search,
                fecha_inicio: filtros.value.fecha.start,
...
/*const cambiarAnioModal = async () => {
    try {
        loadings.value[2] = true;
        await $api("/cambiar-anio-fiscal");
        loadings.value[2] = false;
        isDeleteAnioFiscalVisible.value = false;
    } catch (error) {
        console.log(error);
    }
};*/
        ...
        <VCard title="Filtrar" class="mb-6">
            <VCardText class="d-flex flex-wrap gap-4">
                ...
                <!-- <VBtn
                    ...
                    @click="isDeleteAnioFiscalVisible = true"
                >
                    Cambiar año fiscal
                </VBtn> -->
                <VSpacer />
